...
const state = (over: { claudeIn?: boolean; codexIn?: boolean } = {}): HarnessState => ({
  claude: { name: "claude", installed: true, signedIn: over.claudeIn ?? true },
  codex: { name: "codex", installed: true, signedIn: over.codexIn ?? false },
  updatedAt: Date.now(),
});
...
test("a signed-IN Claude CLI is still not enough without a credential we hold", () => {
  const h = host();
  h.harness.setState(state({ claudeIn: true }));
  assert.equal(h.engine.provider, undefined,
    "the SDK bills against a token we must actually have");
  h.stop();
});
