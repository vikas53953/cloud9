...
The expected full-run count moves from 584 to 589 for these five new checks. The two parity and presence checks are updates to existing checks, not additional count.
...
- Static `ok(` count: 543 at `HEAD`, 548 after this change (`+5`), matching the expected-count increase.
...
- Focused red/green contract probe (in-memory fixtures, not a browser substitute) made all five new predicates fail on deliberately wrong fixtures and pass on the correct fixtures. The harness's normal self-check (`assertHarnessIsHonest`) remains in the existing runner; no broken state is committed.
