...
Last updated: **2026-08-07 — THREADS: he has ANSWERED (both modes, room gives way). The review
...
|---|---|---|---|---|
| 1 | **threads** | **threads-drag-to-resize agent** | **PR #12 OPEN — round 1 REJECTED on nine items (all fixed), round 2 rejected on two (both fixed); reviewer says it approves on sight. DO NOT MERGE. Research only; no app code yet.** | **ROUND 1 REJECTED — and the page had already been published to him, which is how two wrong quotes reached him. The rule that came out of it and is now at the top of this file: a page goes to Vikas only AFTER the reviewer agrees.** The reviewer drove Buzz over a debug port with real mouse events and read the element sizes, re-read the installed stylesheet and re-fetched the Slack pages; its numbers beat the page's and every one of them was re-checked here before being used. **The two bad citations, both fixed and owned out loud on the page rather than quietly swapped:** (1) *"Click and drag the edge of your sidebar"* was quoted under a heading saying the thread pane can be dragged — article `212596808` is **entirely about the LEFT navigation sidebar and never mentions the thread pane or the right-hand side**, re-fetched today to confirm; (2) *"In the top right-hand corner, click the window icon"* is **not in** *Open separate windows in Slack* at all — deleted; the real steps are verbatim *"open a thread. Click the three dots icon. Select Open in new window."* **The source the page had MISSED is now the strongest one on it:** Slack's Accessibility changelog `50668520513939`, June 2026, Desktop — *"You can now resize the secondary view panel and thread pane with arrow keys."* It names the thread pane, it is two months old, **and it exposed a hole in our own design: there was no keyboard way to resize.** Arrow-key resizing is now in the plan. **The worst find was a DESIGN fault, not wording: a DEAD HANDLE on small windows.** At 800px the space beside rail and sidebar is 800−78−216 = **506px** while the old floors (300 thread + 380 room) need **680px**, so the fallback pinned the thread at **232px — below its own floor — and the handle did nothing when dragged, with nothing on screen saying why**. "Share proportionally as today" is his own complaint with a handle bolted on. **Fixed by making the narrow window and the take-over mode the SAME mechanism**, which is what Buzz does: below **894px** (78 rail + 216 sidebar + 300 + 300) the thread takes the whole area with a back arrow. Buzz's own switch is between 800 and 850 — measured, it still splits and drags at 900 AND 850. **The 380px room floor was invented**; measured, Buzz is **symmetric at ~300 both sides (thread 308, channel 301)**, so 380 was costing him 80px of maximum thread for no stated reason. Now 300/300. **Every number corrected:** Buzz default **388** (was 373), narrowest **308** (293), widest **1399** (1384), channel floor **301** (299) — a consistent ~15px shortfall, the divider's own width, lost by measuring off screenshots. Cloud9's own figure was **280, not 268**: `clamp(232px,(1920−78−250)×0.22,280px)` = 1592×0.22 = 350 → capped at 280; 268 only happens at ~1546px. The page had **contradicted the formula it printed four paragraphs later**. Range at his real **1920px viewport** (1936 is the outer window frame) is **300–1292** with the corrected floors; the old "1230" was not even right for its own 380 floor (1592−380 = 1212). **Two "not verified" items had stopped one step early:** split view is now **strong evidence of NO** — Slack scopes it to *"any channel or direct message (DM)"* plus canvases and lists, an otherwise exhaustive positive list with threads absent, while the separate-windows article names threads explicitly. The 2020 tweet's **date** is search-corroborated only (x.com returns 402 to fetchers) and now says so. **Slack's pixel limits and narrow-window behaviour are genuinely undocumented and are labelled so nobody inserts a number.** **The composer justification was contradicted by our own code:** the 300 floor was justified as stopping the button row wrapping — `styles.css:1948` sets `.composer .tools{flex-wrap:wrap}` globally and `:1703` again for the thread composer, with the comment *"a row that cannot wrap is a row that eventually scrolls sideways."* Wrapping is deliberate. Floor kept at 300 because that is Buzz's number; the false reason is retracted on the page. **Two verified details copied exactly:** Buzz's tooltip is **conditional** — `"Drag to resize."` at the default, gaining `"Double-click to reset width."` only once a custom width exists — and persistence has real backing, `App.tsx:1550` `makeStore()` writes to localStorage and `Prefs` already carries `theme`. The load-bearing grep was re-confirmed: **no `onPointerDown`, `pointerdown`, `setPointerCapture`, splitter or resizer anywhere in `apps/desktop/src` or `apps/relay/src`** — only an unrelated `ResizeObserver` and the @-mention list. There was never a handle. **HE HAS ANSWERED ONE OF THE TWO, and the difference is now labelled on the page instead of blurred:** he wrote **"i want both"** — divider AND take-over mode, overruling the recommendation, which is not re-argued. **The room-gives-way rule is NOT a clean yes and round 2 caught it.** He marked the proposal block 👍, but **the block he marked said the room floor was 380px, and this round changed that block to 300px** — so carrying the tick across is an inference dressed up as a confirmation, on the single axis all three previous attempts died on. It is not re-asked (a 380 room floor already means the room gives way, and 300 moves further the way he has been asking) but it is now **labelled in his own words on the page**: *"You marked the earlier version of this block 👍. That version let the room shrink to 380 px; this one lets it shrink to 300 px — same direction, more room for the thread. If that is not what you meant, say so before I start."* An inferred approval with a door out, instead of a silent one. **ROUND 2 — one small pass from approval; both fixes made.** (1) **`1470px` was the one figure never re-taken** — measured over the debug port, Buzz's expanded overlay is **1691px** (left 221, right 1912), out by **221px**, fifteen times the ~15px screenshot slop this page correctly diagnoses everywhere else. It mattered because the corrections block claimed *"every figure on this page has been corrected"* — a blanket claim gone false on the page whose whole authority rests on it. Number fixed, sentence narrowed, and the miss is named rather than quietly mended. It also **strengthens** the take-over case: the gap is **280 → 1691**, not 280 → 1470. (2) the 👍 labelling above. **Tidy-ups ridden along:** *"above 894px it splits and drags as normal"* oversold it — travel is **0px at 894, 6px at 900, ~56px at 950**, and it only feels like a control from ~1000px; Buzz tapers identically, so it is inherent, and the page now says so. **What happens to a saved wide width when the window narrows into the clamped region is now answered** — set 1200 at 1920, drop to 1330 (1036 available, max thread 736) and the 1200 is **kept, drawn clamped, and restored on widening**; only his own drag or a double-click ever rewrites it. Round 1 promised restore, round 2 only promised it below 894 — **the likeliest way the finished feature would have quietly disappointed him.** Buzz's way OUT of take-over is copied as exactly as its tooltip was: `aria-label="Show thread beside channel"`, measured — **not a back arrow at that size**. Three committed screenshots referenced nowhere (`buzz-default`, `buzz-min-width`, `buzz-wide`, ~700KB) deleted. **Round 2 also verified all three of my catches against the reviewer:** 894 is right and its own 974 was stale (computed from the 380 floor it had just deleted — carried forward, **a 94px band of dead handle would have shipped inside the fix for dead handles**); 300–1292 is right; and the `--side-w` breakpoint (`styles.css:82` / `:1576`) is real and it had not stated it. It also measured the restart claim that round 1 asserted with nothing behind it: **the expanded mode really does survive a restart** — set expanded, process killed, relaunched, came back expanded with no divider. Page re-rendered with a script rather than a screenshot: **13,671px tall, 10 sections, 10 feedback handles all wired, footer present, no horizontal overflow, no broken images, no console errors.** *Round-1 findings, swept so no superseded number survives here:* **The pre-PR-#12 diagnosis was WRONG and the shipped fix went the wrong way.** He wants to **drag the thread's edge and make it BIG, and have it stay** — "pull it to the left-hand side like slack". What shipped made it a fixed, narrower slab with a hard 280px ceiling and no handle at all. Three rounds failed for one reason: **nobody opened Slack or Buzz and looked.** This round did. **Slack's own announcement names the thread view:** "You can now resize both sidebars within Slack! Expand the thread view, collapse the channel sidebar entirely" — wording fetched verbatim off the Facebook mirror; the 12 June 2020 date is search-corroborated only. **Buzz 0.5.5 — the reference product installed on this machine — was opened and driven**, not read about: its divider tooltip says **"Drag to resize."**, gaining **"Double-click to reset width."** only once a custom width is set. Re-measured over a debug port at his **1920px viewport**: Buzz's thread runs **308px → 1399px** (the CHANNEL takes the squeeze, floor **301px**), default **388px**, double-click resets, width holds across a channel switch and reopen but is **forgotten on restart**; at an 800px window Buzz **stops splitting entirely** — thread full-width with a back arrow — while still splitting and dragging at 850 and 900. Cloud9 measured in the installed app: **thread 280px** (its ceiling) **and no resizer anywhere in the code**. The one line that says it all: **Buzz's SMALLEST thread (308px) is wider than Cloud9's LARGEST (280px).** Review page with every source and picture: `docs/threads-like-slack.html`. **Nothing gets built until he has seen it** — his new standing rule: "if i am giving you the reference of any product, you just have to open that and see it first and then only implement. from now on you will tell me first what you are implementing." *Superseded diagnosis, kept for the record:* He was never dragging a divider — he was PULLING THE WINDOW. Measured in the installed app: the thread was 15% of the screen at 1920 and 20% at 1440, but **34% at 800px, where it was WIDER than the conversation itself** (268 vs 238px), room title cut to a bare `#`. It was a fixed slab while the room was told to give way to nothing, so the room was the only thing that ever yielded. That is why tuning the width at 1440 twice changed 4px and looked identical. Now: capped at 280px, and **the room is the wider of the two at every window size the app allows**, which it never was. The reviewer swept **700→2560px in 2px steps — 931 measurements** rather than sampling. At 1440: 292 → 244.6px. Before/after pictures at four sizes in `docs/qa/threads-2026-08-07/`. |
