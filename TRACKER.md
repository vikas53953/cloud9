...
`42e1b3c`, and **CLEAN against `origin/master`**. It is not approved or merged;
...
### Merge reconciliation — branch-only PR #13 evidence retained (2026-08-07)

The `fix/agents-are-not-on-a-timer` parent carried a longer author-evidence row
than `origin/master`. The merged board keeps the current master rows 10–12 and
the branch's additional measurements here rather than silently dropping them:

- The four blocker fixes were measured as: two approval-card waits release an
  execution slot; a dropped socket does not answer for the user and an
  approval arriving after restart plainly says nothing happened and asks again;
  captured output keeps the answer tail (16 MB target) and fails loudly when
  the answer is missing; and the `taskstuck` comparison was paired twice.
- The paired control numbers from the branch parent were round 1: branch 11/11
  in 100.4s vs master 11/11 in 97.2s; round 2: branch 9/11 in 324.5s vs master
  8/11 in 326.4s. Both sides degraded together in round 2, so this remains
  machine-sensitive and still needs the clean detached control requested by the
  reviewer.
- The branch parent also disclosed that Stop does not reach `SdkProvider`, and
  that a Codex agent has no spending cap; those are boundaries, not approval
  claims. Its old `shared/index.ts:664` citation is intentionally corrected to
