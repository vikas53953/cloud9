...
|---|---|---|---|---|
| 1 | **threads** - *"pull it to the left-hand side like slack"* | author (Luna) | **MERGED (PR #14, `7ce7bcd`) · BUILT · PACKAGED · INSTALLED · MEASURED.** | Round-1 width/inert findings remain fixed. Round-2 focus fix retains the initiating reply control, restores it after beside/back, forced 800px back, and responsive unforce when it remains visible, and uses a visible room-control fallback when the reply action is no longer visible; close no longer falls to `<body>`. Installed evidence from `%LOCALAPPDATA%\\Programs\\Cloud9\\Cloud9.exe` (SHA256 `CB0018B75455A29090CC6386A9EBBFD04B0F680B5ED92575CD23C5E721E3E52E`), renderer `index-DKWyPRcf.js` (SHA256 `23C2019264327BB6B85B318CF54EF8B9EC26396865235F5EA637F6AA824D0245`) / `index-shSU9Pdt.css`, is in `docs/qa/threads-drag-2026-08-07/measured.txt` (run 2026-08-07 17:03 +05:30). Measured acceptance map: startup 388 with divider aria 300/388/1292; pointer drag 1100, 340 and 1292 with room floor 300 and pointer cleanup; arrow keys 696 then 648; tooltip changes only after a custom width; take-over dims the room and returns with `show thread beside channel`; actual 40-Tab focus isolation at wide and forced 800; exact focus restoration to the initiating reply after wide back, responsive 800→1330 unforce, and forced 800 back; close restoration to the initiating reply; 894 exact 300/300 with aria 300/300/300; stored 900 survives 1330, 800, 1920 and a real restart. Desktop source tests 29/29, build/typecheck, dist, install and dedicated installed walk pass. Full 
pm test`, 
pm run qa`, and generic 
pm run qa:app` remain UNKNOWN from prior bounded runs; no full-suite or generic-walk pass is claimed. Rebased branch includes round-2 commit `6d1a942` on origin/master `7ce7bcd`. PR #14 was independently reviewed and merged; full 
pm test`, 
pm run qa`, and generic 
pm run qa:app` remain UNKNOWN from prior bounded runs.
...
otimers.test.ts` 7/7, `spendplanfallback` 28/28, shared 218/218, full build
...
| 15 | **hooks editor** | nobody yet | **NOT STARTED — verified buildable harness gap; author/reviewer unassigned** | The rules engine is wired at `packages/engine/src/hookwiring.ts:63-78`, and four event kinds are defined at `packages/engine/src/hooks.ts:52-62`, but no screen or relay frame lets the owner create/list/edit/delete a rule. `rg` over `apps/desktop/src` and `apps/relay/src` finds no `HOOK_EVENTS`, `saveHooks`, 
ewHookId`, or `addHook` caller. Boundary: build an owner-only plain-words editor for the existing schema/events/actions, with visible refusal/firing history; do not invent new hook powers. Dependencies: none. |
| 16 | **saved-key provider parity** | nobody yet | **NOT STARTED — verified buildable harness gap; author/reviewer unassigned** | When a stored API key/token exists, `packages/engine/src/host.ts:184-189` selects `SdkProvider`; `packages/engine/src/provider.ts:705-780` runs `maxTurns: 6`, supplies no Cloud9 MCP doorway/live steps, and has no registered process for Stop. The same editor still says switches are in use. Boundary: align the saved-key route with the existing agent contract for streaming, Stop/process-tree ownership, Cloud9 tools, and honest outcome records. Key ownership, sessions beyond this route, and paid/account decisions remain outside this row. Dependencies: none; this row is independent of queue visibility. |
