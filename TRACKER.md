...
| 2 | **agent activity and details** | agent — PAUSED ON QUOTA | **PR #9 · review done · CHANGES REQUESTED · 8 faults being fixed** | It correctly did NOT build a second screen. The existing Activity screen was structurally incapable of "now": it never read `agentStatus`, and the hub sends activity **as a reply, never a push**, so the trail froze the moment he opened it. The new "Right now" board IS genuinely live — the reviewer traced the push path and confirmed it. But it **lies in eight ways**, and two are this app's own history repeating: a **paused agent reads ✅ Finished** (the Stop-says-finished bug, recreated), and **the rail count and the board contradict each other on the same screen** when an agent stops to ask him something. Queued work is invisible. The two states that matter most — 🛑 and "waiting for you" — were never photographed, and three faults live there. |
