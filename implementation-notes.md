...
- 2026-08-15 Chat View modes (Agent 4): Chat + Files was unmounting whenever a thread or room details opened (`ChatScreen` gated the panel and `withworkspace` on `!threadRoot && !detailsOpen && !takeover`), so a click on a message looked like a reset to Focused Chat even though `prefs.workspaceLayout` stayed chat-files. Panel now stays mounted; thread/details stack in a fourth column. View select `:focus` uses full ink (selected ≠ disabled).
