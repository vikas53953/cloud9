...
- FEATURE THREADS DONE-PROVEN 2026-08-04. THE REAL BUG Vikas reported: agents only ever talked in the channel - engine.ts agentSend never passed replyTo, though the wire supported it all along. Fixed at TWO seams not 35 call sites: TurnInput.replyTo (engine.ts:104) carries the thread on the TURN, and agentSend(.., {proactive?, replyTo?}). One owner packages/engine/src/threads.ts (threadOf + roomLineForThreadJob). Long work started in a thread reports detail INTO the thread plus one short room line (the decision recorded in feature-gap.md:300 and Buzz's own broadcast-flag behaviour). Deliberately channel-level: schedule firings (no message to answer), received handoffs (contextPointer names only a channel), proactive/presence, and anything asked in the room. Screen half: thread_reply notify kind (participants only, mute silences it, a mention never double-fires), the room row shows when the new thing is only in a thread, the replies line reads New until opened, thread composer regained its tools. Evidence: 1,170 tests 0 fail; qa 542/542+8/8+4/4; installed walk 35/35 (once).
- HONEST LIMITS logged, not faked: the thread "New" mark lasts the visit (surviving restarts needs a per-thread read marker on the hub); an agent answering in a thread still gets the WHOLE ROOM's recent history as context, not the thread's; isThreadParticipant is applied to the toast but NOT to the on-screen marks, so a stranger's thread can still put a mark on your row (decision needed).
- AGENTIC GAP scouted for the "make Cloud9 like Codex" ask. ALREADY DONE (do not rebuild): full tool ceiling incl. Bash/PowerShell with alwaysAsk gating, connections MCP picker, git worktree per agent per job with approval-gated publish, the complete step-by-step run record, per-agent memory. THE REAL GAPS ranked: (1) NO SESSION RESUME - sessionId/thread_id is captured then never used; every turn is a cold `claude -p`/`codex exec`, so a long task is re-explained from a transcript each time (ARCHITECTURAL, the biggest felt difference); (2) NO LIVE STREAMING - run.ts has no stdout streaming hook, the whole trace lands only when the process exits, so "What it did" is post-hoc (SLICE; mappers already parse per-line); (3) wholeComputer folder picker STILL INERT (host.ts:144 says so outright) - cheapest fix, and the one switch on the ceiling that currently lies; (4) background jobs are killed at 2-3 min (claude-cli.ts:439 / codex.ts:559 defaults, no per-kind override) which undercuts the whole delegated-work story; (5) no file/image attachments.
- CHAT EXPERIENCE round (branch sol/chat-experience), built overnight 2026-08-04, NOT yet through the conductor's chain:
  * SMOOTHNESS: one owner selector layer in store.ts (subscribeTo/useWorld + shallowEqual, :3096-3136); MessageRow is React.memo and subscribes to NOTHING (props only, App.tsx:4843); ChatView/ChatScreen read slices; unreadFor keeps a per-room ref cache so an unrelated room's message no longer re-scans every room. MEASURED in a real Electron mount, 150 messages: one incoming message 151 MessageRow renders -> 1; an unrelated presence/status tick 151 -> 0; one reaction 151 -> 1. QA hook window.__cloud9Renders kept deliberately. SKIPPED WITH REASONS: history cap (loadOlder's cursor points before the oldest held message; trimming would leave a HOLE - needs a relay-side cursor, own slice) and optimistic send (loopback hub already clears instantly; the real win is only over a friend's network - own slice).
  * SEMANTIC RECEIPTS (his draft s2): ephemeral only - never stored, never in search, never unread. shared/receipts.ts owns the vocabulary; engine/receipts.ts turnVerdict() is ONE pure function deriving the verdict from FACTS (cancelled->none; failed->conflict; clean but empty reply->none, never a cheerful tick; reply's last line ends in ? -> needsInput; any read/search/web step -> investigating; else agreed). Hub forwards via the existing audienceFor owner and writes NOTHING. Screen: non-interactive span, greyed/smaller, tooltip says it is a live app signal and is not saved, so it can never read as a human reaction. reading/thinking expire after 5 min so a crashed engine cannot leave a thinking bubble spinning forever.
  * TIME BUDGETS: one table (engine/timebudget.ts) keyed on promptTurnKind - chat 3 min, task/schedule/repo 30 min, hard ceiling 60. Before: Claude 180s vs Codex 120s with no reason recorded, and NOTHING overrode them per kind, so delegated work died at 2-3 min. The timeout is now its own error allowed through the sanitiser whole, so the person is told it ran out of time and how much it had, in Task.error too. CATCH WORTH REMEMBERING: !code typed in a room is recorded as RunKind "chat" (engine.ts:1488) - any future per-kind policy must read promptTurnKind, not RunKind, or repository work gets the chat clock.
- THREADS ROUND 2 (the real ask) 2026-08-04: my miss - I built only "asked inside a thread -> answered there" and even wrote a test asserting a channel question should stay in the channel. Vikas tested @agent in a channel and it flattened, exactly as he reported. FIX: threads.ts threadOf() now returns trigger.replyTo ?? trigger.id, so a channel question becomes its own thread root (idempotent through the hub's one-level resolveReplyTo). Proven on the INSTALLED app with screenshots (docs/qa/thread-channel-question-*.png). Prefs.replies is a DRAWING choice, not routing, so his existing "inline" setting is already the off-switch.
- CONSEQUENCE THAT BROKE 15 CHECK SITES: agent answers are no longer rows in .msgs, so every QA wait-for-agent-reply timed out (all three scripts died). Fixed as a class: ONE helper waitForAgentAnswer() in scripts/qa-target.mjs, used by all four scripts; the old waitForAgentReply is DELETED so nothing can look in the old place. Trap it now guards: a job's own room line QUOTES the ask and sits below it, so find-by-words picked the agent's line and blamed the feature.
- LIVE STEPS: run.ts gained an onStdoutLine hook; traceFromStream was REFACTORED (not copied) into one traceWalker fed either a whole transcript or line by line, so the live preview and the stored record cannot drift - a test asserts they are deep-equal on both real fixtures. Ephemeral like receipts: hub forwards, stores nothing. Falls back silently when streaming is unavailable; no empty box.
- WHOLECOMPUTER PICKER: the last inert switch is closed (inertSwitches is now []). One owner packages/engine/src/wholecomputer.ts resolves off/none/gone/partly/ready/unchecked; host.ts:158 supplies --add-dir; relay's refuseAgentPathsFromEngine covers both path fields.
- SCROLL ROOT CAUSE (the real one): FOLLOW_SETTLES_MS=700 was treated as how long a follow LASTS, but a smooth scroll grows with distance (745ms at 2,000px, ~1,520ms at 13,000px). The timer fired mid-animation, following.current went false, and every remaining step of the app's OWN animation set atBottom=false, switching off both the arrival rule and the resize watcher - landing short by exactly his row's height (267px with a 12-line message, 0px of it visible). Now the timer measures STILLNESS. 7/13 runs broken before, 0/13 after. The FIRST diagnosis (a stuck load anchor) was TESTED AND FALSE - the false story has been removed from both the code comment and qa.mjs's prose.
- OPEN DECISIONS FOR VIKAS: (1) DMs now thread every answer too - may feel heavy in a one-to-one; (2) NOTIFICATION VOLUME - every agent answer is now a reply in his own thread, so threadReplyEvent toasts once per answer; my recommendation is to suppress thread_reply when the reply answers HIS OWN question (he asked for it, it is not news); (3) a channel-started !bg now puts detail in the thread and one "Finished in the thread" line in the room.
- 2026-08-05 "Claude/Codex not installed" on the installed app - ROOT CAUSE: Cloud9 looked at this computer EXACTLY ONCE, at engine start-up (host.ts:263 engine.onReady), and never again. Whatever that single look said stood forever. His `npm i -g` rewrote claude.cmd at 23:27 while the app was looking, so a momentary absence became a permanent accusation ("not installed on this computer", sign-in greyed via disabled={!installed}). A timed-out probe was reported with the SAME wording as a genuinely missing app (cold `claude --version` measures 5-6s, codex 8.6s, against a 20s leash on a busy machine). NOT caused by commit 82cdaf6 - verified: the packaged app always mints its own 43-char key (main.cjs:216 ensureOwnerToken) so the DEFAULT_OWNER_TOKEN guard cannot fire there.
  FIX AS A CLASS: harness.ts:440 scheduleRelook() - one look is never the last word (5s/15s/30s/1m/2m then every 5m while anything looks missing, every 10m when healthy, unref'd, waits for an in-flight sign-in); harness.ts:117 askVersion() gives a timeout one second chance at double the leash; a timeout is no longer worded as an absence. Proved on the installed app: blind at start, recovered by itself at 72s with nobody pressing anything.
  WHY 36/36 MISSED IT: the walk only ever asserted the GitHub card on Settings - the two cards that decide whether ANY agent can run were never looked at, and the walk only ever asked the app about itself so it could not call a card a liar. Now 38 checks: one for "the app says what it found instead of checking forever", one that runs `claude --version`/`codex --version` ITSELF and fails in either direction. Proven red with the app deliberately blinded.
  ALSO FOUND: commit 82cdaf6 (owner key out of localStorage) broke drive-app.mjs, which still read localStorage for the key - 12 checks failed on the fresh build. Fixed to read the preload bridge.
- 2026-08-15 Studio Floor engineering UX pass (branch ux/studio-floor-polish): helm owns coordination because nearly all UI lives in one App.tsx. Specialist agents have exclusive function ranges so they do not blindly rewrite the same components. Chat+Files click-away was already removed (PR #47); remaining P0 is the panel unmounting when threadRoot/detailsOpen/takeover is set even though prefs.workspaceLayout stays chat-files. Channel drag-reorder already exists (useDragReorder); there are no visible ↑↓ on channel rows. Composer empty min-height is 40px (thread 36px); polish no longer re-raises it to 68px. Auto-grow via scrollHeight is unchanged. Turn into task moved into the message More menu so it is no longer a grid sibling overlapping threadLine.
- 2026-08-15 Crew cards (Agent 6): square poster plates repeated Ready + provider chips + "signed in to Claude". One flag from presenceSays/waitingOn; runtime is a muted line; portraits sit 118px beside facts (`.cast.crewcast`). Did not invent "Needs approval" — still "Waiting on you".
- 2026-08-15 Settings / Integrations: Connected apps stays account + sign-in; CLI versions, runtime, model discovery and connection probes live under Advanced / Diagnostics (SettingsDiagnostics). HarnessHonesty stays on the agent editor, not Settings. Card facts (.harnessfacts, .modelsource, .checkedline) remain in the card DOM so existing QA walks still read them.
- 2026-08-15 Workflows screen (Agent 10): list/detail hid purpose, agents, trigger, current step, failures, approvals, and outputs behind a name + raw channel id + "v1" asides. Words now live in packages/shared/src/workflow-view.ts; the screen only draws. Step ↑↓ in the editor stays for step order, not channel reorder.
- 2026-08-15 Design system (Agent 13): greyed-after-click was cascade, not pigment. Shared `.iconbtn` still hands out `--rail-muted` (specificity test), but hover/pressed on canvas now use `--ink`/`--surface-2` and `.stage .iconbtn` outweighs the rail base. Enabled chips/iconbtns no longer use disabled opacity. `.chathead select` / `.input` strip OS appearance. Primitives section owns type, badges, empty/error/loading, button hierarchy. Feature layouts untouched. `theme-appearance-specificity.test.cjs` 6/6.
- 2026-08-15 Files (Agent 7): empty FileRelations no longer draws "How this file connects"; QA still sees `data-relations-state="empty"` on `.files-detail-inner`. Copy newest/exact and raw ids live under References and IDs. Index says "from a turn" and keeps the run id in `.file-index-maker` for the walk.
