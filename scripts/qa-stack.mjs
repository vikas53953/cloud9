...
/**
 * How long a leftover QA workspace may sit before this script sweeps it.
 * Younger siblings belong to a parallel run and must never be deleted.
 */
export const QA_WORKSPACE_MAX_AGE_MS = 3 * 60 * 60 * 1000;

/**
 * Decide whether a `cloud9-qa-*` temp folder should be deleted.
 *
 * Own workspace: always delete (this run's cleanup).
 * Sibling younger than the max age: keep — another QA run is using it.
 * Sibling older than the max age: delete — abandoned litter.
 */
export function shouldDeleteQaWorkspace({
  name,
  isOwn,
  ageMs,
  maxAgeMs = QA_WORKSPACE_MAX_AGE_MS,
}) {
  if (!name.startsWith("cloud9-qa-")) return false;
...
// So every run starts by clearing ABANDONED QA workspaces (older than 3 hours).
// It must never delete a younger sibling — two QA runs at once used to wipe
// each other's databases mid-run.
function sweepStaleWorkspaces(ownWorkspace) {
  let swept = 0;
  let entries = [];
  ...
  for (const entry of entries) {
    ...
    const isOwn = ownWorkspace !== undefined && path.resolve(full) === path.resolve(ownWorkspace);
    let ageMs = Number.POSITIVE_INFINITY;
    try { ageMs = now - fs.statSync(full).mtimeMs; } catch { continue; }
    if (!shouldDeleteQaWorkspace({ name: entry, isOwn, ageMs })) continue;
    try { fs.rmSync(full, { recursive: true, force: true }); swept++; }
